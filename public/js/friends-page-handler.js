...
(() => {
	...
	function initFriendsPage() {
		...
		var pagination = document.getElementById("friends-pagination");

		// 如果关键元素不存在，直接返回
		if (!searchInput || !friendsGrid || !noResults || !pagination) {
			return false;
		}
		...
		var previousPageLabel =
			pagination.getAttribute("data-previous-label") || "上一页";
		var nextPageLabel = pagination.getAttribute("data-next-label") || "下一页";
		var pageLabelTemplate =
			pagination.getAttribute("data-page-label") || "第 {page} 页";
		...
		function renderPagination(totalPages) {
			...
			previousButton.className = "friends-page-btn";
			previousButton.textContent = previousPageLabel;
			...
			for (var i = 1; i <= totalPages; i++) {
				...
				pageButton.className =
					"friends-page-btn" + (i === currentPage ? " active" : "");
				pageButton.textContent = i;
				pageButton.setAttribute(
					"aria-label",
					pageLabelTemplate.replace("{page}", i),
				);
			...
			nextButton.className = "friends-page-btn";
			nextButton.textContent = nextPageLabel;
		...
		function filterFriends() {
			...
			var pageCards = matchingCards.slice(startIndex, endIndex);
