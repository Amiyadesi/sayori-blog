...
- **新しい投稿を作成：** `pnpm new-post <slug> [title]` は `../articles/posts/<slug>/<slug>.md` を作成します
- **投稿を編集：** `../articles/posts/` のソースを編集します。`src/content/posts/` は同期生成物です
...
- **published**: 公開日（必須）
- **created**: ソースファイルの作成日
- **updated** / **lastEdited**: 最終編集日。公開フローが更新します
- **updateCount**: 公開時に記録する内容更新回数
...
| `pnpm new-post <slug> [title]` | `../articles/posts/` にソース投稿を作成 |
