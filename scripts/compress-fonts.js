...
function addTextToSet(textSet, value) {
	if (!value) {
		return;
	}
...
function readLocalMusicPlaylistText() {
	...
	try {
		const music = JSON.parse(fs.readFileSync(musicConfigPath, "utf-8"));
		const tracks = Array.isArray(music.tracks) ? music.tracks : [];

		for (const track of tracks) {
			addTextToSet(textSet, track.title);
			addTextToSet(textSet, track.artist);
			addTextToSet(textSet, track.category);
		}

		if (tracks.length > 0) {
			console.log(`✓ Read ${tracks.length} songs from articles/site/music.json`);
		}
	} catch (error) {
		console.log(
			`⚠ Error reading local music playlist: ${error.message}, skipping local music text collection`,
		);
	}
...
async function fetchMetingPlaylistText() {
	try {
		const localTextSet = readLocalMusicPlaylistText();
		...
		const textSet = new Set(localTextSet);
		...
		try {
			...
			playlist.forEach((song) => {
				...
				if (title.trim() || artist.trim()) {
					...
					// 添加歌名中的字符
					addTextToSet(textSet, title);

					// 添加歌手名中的字符
					addTextToSet(textSet, artist);
				}
			});
			if (songCount === 0) {
			...
			if (fetchError.name === "AbortError") {
				...
				console.log(
					`ℹ Failed to fetch Meting API data: ${fetchError.message}, using local music text only`,
				);
			}
		}
