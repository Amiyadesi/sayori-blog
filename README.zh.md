...
- **创建新文章：** `pnpm new-post <slug> [标题]` 会创建 `../articles/posts/<slug>/<slug>.md`
- **编辑文章：** 修改 `../articles/posts/` 中的源文件；`src/content/posts/` 是同步产物
...
- **published**: 发布日期（必需）
- **created**: 源文件创建日期
- **updated** / **lastEdited**: 最后内容编辑日期，由发布流程更新
- **updateCount**: 发布时记录的内容更新次数
...
| `pnpm lint`                | 检查并修复代码问题                     |
| `pnpm new-post <slug> [标题]` | 在 `../articles/posts/` 创建源文章      |
