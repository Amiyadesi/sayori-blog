(() => {
	...
	const MERMAID_SCRIPT_SOURCES = [
		"/assets/js/mermaid-11.12.1.min.js",
		"https://cdn.jsdelivr.net/npm/mermaid@11.12.1/dist/mermaid.min.js",
		"https://unpkg.com/mermaid@11.12.1/dist/mermaid.min.js",
	];
	...
	function getSvgIntrinsicSize(svgElement) {
		try {
			...
			console.warn("Unable to measure Mermaid SVG bounds:", error);
		}
	...
	function fitDiagramToViewport(element, wrapper, svgElement) {
		const size = getSvgIntrinsicSize(svgElement);
		const width = Math.max(240, size.width);
		const height = Math.max(120, size.height);
		const availableWidth = Math.max(
			240,
			(element.clientWidth || element.parentElement?.clientWidth || 720) - 16,
		);
		const availableHeight = Math.max(
			220,
			Math.min(680, window.innerHeight * 0.76),
		);
		const heightFitScale = availableHeight / height;
		const normalizedScale = Math.min(1, Math.max(0.72, heightFitScale));
		const scaledWidth = width * normalizedScale;
		const shouldOverflowX = scaledWidth > availableWidth;
		const viewportHeight = Math.ceil(height * normalizedScale);
		const tx = shouldOverflowX
			? 0
			: Math.max(0, (availableWidth - scaledWidth) / 2);
		...
		element.classList.add("is-mermaid-rendered");
		element.style.minHeight = `${Math.min(Math.max(viewportHeight + 52, 220), 720)}px`;
		if (shouldOverflowX) {
	...
	function attachZoomControls(element, svgElement) {
		...
		const initialViewport = fitDiagramToViewport(element, wrapper, svgElement);

		let scale = initialViewport.scale;
		let tx = initialViewport.tx;
		let ty = initialViewport.ty;
		const MIN_SCALE = 0.2;
		const MAX_SCALE = 6;
		...
		controls.addEventListener("click", (ev) => {
			...
			switch (action) {
				...
				case "zoom-out":
					...
					break;
				case "reset":
					({ scale, tx, ty } = fitDiagramToViewport(
						element,
						wrapper,
						svgElement,
					));
		...
		window.addEventListener("resize", () => {
			...
			resizeTimer = setTimeout(() => {
				const fitted = fitDiagramToViewport(element, wrapper, svgElement);
				scale = fitted.scale;
				tx = fitted.tx;
				ty = fitted.ty;
				applyTransform();
					...
					batch.map(async (element, localIndex) => {
						...
						while (attempts < maxAttempts) {
							try {
								...
								element.classList.remove("is-mermaid-rendered");
	...
	async function loadMermaid() {
		...
		return new Promise((resolve, reject) => {
			...
			function appendScript() {
				...
				script.src = MERMAID_SCRIPT_SOURCES[sourceIndex];
				script.async = true;
				...
				script.onerror = (error) => {
					...
					if (sourceIndex < MERMAID_SCRIPT_SOURCES.length) {
						appendScript();
						return;
					}
