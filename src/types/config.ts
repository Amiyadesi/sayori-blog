...
export interface AnnouncementConfig {
	...
	id?: string; // 当前公告唯一标识，变更后重新提醒
	updated?: string; // 当前公告更新日期，YYYY-MM-DD
	title?: string; // 公告栏标题
...
export interface ThirdPartyAnalyticsConfig {
	...
	umami?: {
		enable: boolean; // 是否启用自托管 Umami tracker
		src: string; // tracker script URL, for example https://stats.sayori.org/script.js
		websiteId: string; // Umami data-website-id, not a secret
		hostUrl?: string; // optional data-host-url when script and collector host differ
		domains?: string; // optional data-domains allowlist
	};
}
