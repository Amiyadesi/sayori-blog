<script lang="ts">
	...
	let externalFrameElement: HTMLIFrameElement | null = null;
	...
	function externalFrameRef(node: HTMLIFrameElement) {
		externalFrameElement = node;
		musicPlayerStore.bindExternalFrame(node);
		return {
			destroy() {
				if (externalFrameElement === node) {
					externalFrameElement = null;
				}
				musicPlayerStore.unbindExternalFrame(node);
			},
		};
	}
...
{#if shouldRenderFloatingUi}
	...
	{#if useFabEntry}
		{#if state.externalPlayerUrl && state.isPlaying}
			<div
				class="music-provider-frame-anchor fixed z-[54]"
				aria-hidden="true"
			>
				<div
					class="music-provider-frame-shell"
					class:provider-narrow={state.externalProvider ===
						"netease"}
				>
					<iframe
						use:externalFrameRef
						class="music-external-frame"
						title="External music provider"
	...
	<style>
		...
		.music-provider-frame-anchor .music-provider-frame-shell {
			position: static;
			margin: 0;
		}
		...
		.music-player-fab-shell {
			...
			animation: fabPanelEnter 280ms cubic-bezier(0.22, 1, 0.36, 1)
				both;
		}

		@keyframes fabPanelEnter {
			from {
				opacity: 0.12;
