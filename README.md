...
Production source for [blog.sayori.org](https://blog.sayori.org). The content
source is the public [sayori-articles](https://github.com/Amiyadesi/sayori-articles)
repository and is synchronized before every build. This repository contains the
theme, Astro app, Cloudflare Pages Functions, and deployment workflow.

```bash
git clone https://github.com/Amiyadesi/sayori-blog.git
git clone https://github.com/Amiyadesi/sayori-articles.git content
cd sayori-blog
pnpm install --frozen-lockfile
CONTENT_DIR=./content pnpm run build
```

`CONTENT_DIR` overrides the default local `../sayori-articles` content path. Generated
...
- **Create new posts:** `pnpm new-post <slug> [title]` creates `../sayori-articles/posts/<slug>/<slug>.md`
- **Edit posts:** Modify files in `../sayori-articles/posts/`; `src/content/posts/` is generated by sync
...
- **created**: Source creation date
- **updated** / **lastEdited**: Last content edit date, updated by the publish flow
- **updateCount**: Count of publish-time content updates
...
| `pnpm new-post <slug> [title]` | Create a source post in `../sayori-articles/posts/` |
